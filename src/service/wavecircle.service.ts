...
export function createOptionsForWaveCircle(
  frequencyIncr: number,
  isCircleSpike?: boolean
): WaveCircleOptions & Dim {
...
export function waveCircleFormation(
  ...
  for (const [startRange, endRange] of bandRanges) {
    ...
    const indexIncrement = totalBands / waveCounts;
    let perBandValue = 0;
...
export function circleSpikeFormation(
  ...
  const anglePerBar = Math.PI / (options.bandRanges.length * options.waveCounts);
